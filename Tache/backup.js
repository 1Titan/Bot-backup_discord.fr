    ...
    async run(g2=null, owner=null) {
      ...
      if (g2 !== null) {
        ...
        if (!found) {
          ...
          let e = new MessageEmbed()
          .setColor(0xff0050)
          .setDescription(`📱Voici votre backup ID : \`${g.backupID}\`.\nConservez-le en lieu sûr au cas où vous perdriez l'accès à ce chat.`)
          if (owner && owner.presence.clientStatus.mobile !== undefined) {
            e.setDescription(`📱Voici votre backup ID.\nConservez-le en lieu sûr au cas où vous perdriez l'accès à ce chat. Pour le copier, appuyez et maintenez le code de sauvegarde et cliquez sur \`Copier le Message\`.`)
            await owner.send(e);
            owner.send(g.backupID);
          } else if (owner) owner.send(e);
          else id = g.backupID;
