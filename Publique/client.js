...
let x = confirm('Êtes-vous sûr de vouloir effacer les données de guilde actuelles ?');
if (!x) return;
...
function backup() {
let guildid = location.href.split('/info/')[1];
let x = confirm('Voulez-vous vraiment effacer les sauvegardes existantes ?');
if (!x) return;
var client = new HttpClient();
...
if (location.href.includes('info')) {
  ...
  client2.get(`/api/guildinfo/${encodeURIComponent(guildid)}?tokens=${encodeURIComponent(localStorage.tokens)}`, function(response) {
    ...
    main.innerHTML = "";
    if (response.error) {
      if (response.error === "Vous n'avez pas assez de perms à afficher les backups.") document.getElementById('form').innerHTML = `<h2>Error</h2><p>You need <code>Manager Server</code> permissions to edit this server.</p>`
...
client.get(`/api/info?tokens=${encodeURIComponent(localStorage.tokens)}`, function(response) {
  response = JSON.parse(response);
  console.log(response);
  if (response.message && response.message === "Votre tarif est limité.") {
    return setTimeout(() => {
      location.reload()
    }, 2000)
  }
