...
app.get("/api/install/:guildid", async (req, res) => {
  ...
  if (!find)
    return res.json({
      error: "Vous n'êtes pas dans cette guilde."
    });
  let perms = new Discord.Permissions(find.permissions).has("MANAGE_GUILD");
  if (!perms)
    return res.json({
      error: "Vous n'avez pas assez de perms pour installer des backup."
    });
...
app.get("/api/backup/:guildid", async (req, res) => {
  ...
  if (!perms)
    return res.json({
      error: "Vous n'avez pas assez de perms pour backup cette guilde."
    });

  let guild = client.guilds.cache.get(req.params.guildid);
  let r = await client.tasks.find(x => x.name === "backup").run(guild);
  res.json({
    result: `Guilde backup avec succès. Votre id de sauvegarde est: ${r}`
  });
});
...
app.get("/api/guildinfo/:id", async (req, res) => {
  ...
  setTimeout(async () => {
    ...
    if (!Array.isArray(guilds))
      return res.json({
        error: "Vous êtes limité par les ratios. Recharger la page après quelques secondes."
      });
    ...
    if (!perms)
      return res.json({
        error: "Vous n'avez pas assez de perms pour afficher les backup."
      });
